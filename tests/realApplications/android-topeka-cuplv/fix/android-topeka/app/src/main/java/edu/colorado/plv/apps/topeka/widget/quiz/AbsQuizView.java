...
import edu.colorado.plv.apps.topeka.activity.QuizActivity;
import edu.colorado.plv.apps.topeka.model.Category;
import edu.colorado.plv.apps.topeka.model.quiz.Quiz;
import edu.colorado.plv.apps.topeka.widget.fab.DoneFab;
import edu.colorado.plv.apps.topeka.widget.fab.FloatingActionButton;
...
public abstract class AbsQuizView<Q extends Quiz> extends FrameLayout implements
    ...
    public AbsQuizView(Context context, Category category, Q quiz) {
        ...
        mQuiz = quiz;
        mCategory = category;
        mSpacingDouble = getResources().getDimensionPixelSize(edu.colorado.plv.apps.topeka.R.dimen.spacing_double);
        mSubmitAnswer = getSubmitButton(context);
        mLayoutInflater = LayoutInflater.from(context);
        mMinHeightTouchTarget = getResources()
                .getDimensionPixelSize(edu.colorado.plv.apps.topeka.R.dimen.min_height_touch_target);
    ...
    private void setUpQuestionView() {
        mQuestionView = (TextView) mLayoutInflater.inflate(edu.colorado.plv.apps.topeka.R.layout.question, this, false);
        mQuestionView.setText(getQuiz().getQuestion());
    }

    private LinearLayout createContainerLayout(Context context) {
        LinearLayout container = new LinearLayout(context);
        container.setId(edu.colorado.plv.apps.topeka.R.id.absQuizViewContainer);
    ...
    private View getInitializedContentView() {
        ...
        quizContentView.setId(edu.colorado.plv.apps.topeka.R.id.quiz_content);
        ...
        setMinHeightInternal(quizContentView, edu.colorado.plv.apps.topeka.R.dimen.min_height_question);
    ...
    private void addFloatingActionButton() {
        final int fabSize = getResources().getDimensionPixelSize(edu.colorado.plv.apps.topeka.R.dimen.size_fab);
        int bottomOfQuestionView = findViewById(edu.colorado.plv.apps.topeka.R.id.question_view).getBottom();
    ...
    private FloatingActionButton getSubmitButton(Context context) {
        if (null == mSubmitAnswer) {
            ...
            mSubmitAnswer.setId(edu.colorado.plv.apps.topeka.R.id.submitAnswer);
    ...
    protected void submitAnswer() {
        submitAnswer(findViewById(edu.colorado.plv.apps.topeka.R.id.submitAnswer));
    }
    ...
    private void performScoreAnimation(final boolean answerCorrect) {
        ...
        final int backgroundColor = getResources().getColor(answerCorrect ?
                edu.colorado.plv.apps.topeka.R.color.green : edu.colorado.plv.apps.topeka.R.color.red);
        final int imageResId = answerCorrect ? edu.colorado.plv.apps.topeka.R.drawable.ic_done : edu.colorado.plv.apps.topeka.R.drawable.ic_fail;
        ...
        final ObjectAnimator iconAnimator = ObjectAnimator
                .ofArgb(mSubmitAnswer, "imageResource", edu.colorado.plv.apps.topeka.R.drawable.ic_done, imageResId);
        ...
        setElevation(getResources().getDimension(edu.colorado.plv.apps.topeka.R.dimen.elevation_header));
