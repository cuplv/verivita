...
public class FillBlankQuizView extends TextInputQuizView<FillBlankQuiz> {
    ...
    private View getStartEndView(String start, String end) {
        LinearLayout container = (LinearLayout) getLayoutInflater().inflate(
                edu.colorado.plv.apps.topeka.R.layout.quiz_fill_blank_with_surroundings, this, false);
        mAnswerView = (EditText) container.findViewById(edu.colorado.plv.apps.topeka.R.id.quiz_edit_text);
        mAnswerView.addTextChangedListener(this);
        mAnswerView.setOnEditorActionListener(this);
        TextView startView = (TextView) container.findViewById(edu.colorado.plv.apps.topeka.R.id.start);
        setExistingContentOrHide(startView, start);

        TextView endView = (TextView) container.findViewById(edu.colorado.plv.apps.topeka.R.id.end);
