...
public class AlphaPickerQuizView extends AbsQuizView<AlphaPickerQuiz> implements
    ...
    protected View createQuizContentView() {
        ScrollView layout = (ScrollView) getLayoutInflater().inflate(
                edu.colorado.plv.apps.topeka.R.layout.quiz_layout_picker, this, false);
        mCurrentSelection = (TextView) layout.findViewById(edu.colorado.plv.apps.topeka.R.id.seekbar_progress);
        mCurrentSelection.setText(getAlphabet().get(0));
        mSeekBar = (SeekBar) layout.findViewById(edu.colorado.plv.apps.topeka.R.id.seekbar);
    ...
    private List<String> getAlphabet() {
        if (null == mAlphabet) {
            mAlphabet = Arrays.asList(getResources().getStringArray(edu.colorado.plv.apps.topeka.R.array.alphabet));
        }
        return mAlphabet;
    }
}
